...
peer lifecycle chaincode queryinstalled -O json | jq -r '.installed_chaincodes | .[] | select(.package_id|startswith("simple_2.0:"))' > ccstatus.json
...
#   --name simple --version 2.0 --init-required --sequence $SEQUENCE --tls \
